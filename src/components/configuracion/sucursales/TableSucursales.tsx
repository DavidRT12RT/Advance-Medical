...
const TableSucursales = ({ form }: any) => {
  ...
  const eliminarSucursal = async (record: any) => {
    try {
      ...
      const sucursal = { ...record };

      dispatch(setLoadingTable(true));
      
      const result = await FireStoreConfiguracion.eliminarSucursal(auth?.empresa?.id, sucursal?.id);
      if (result) {
        Swal.fire({
          position: "top-end",
          icon: "success",
          title: `Sucursal eliminado con éxito!`,
      ...
      Swal.fire({
        title: "Error",
        text: "No se pudo eliminar sucursal",
        icon: "error",
      });
    }
  };
  ...
  const columns: TableProps<any>['columns'] = [
    {
      title: <Typography.Title level={5}>{`Lista de sucursales, ${listaDeSucursales.length} resultados`}</Typography.Title>,
      dataIndex: 'catalogo',
      key: 'catalogo',
      children: [
        // {
