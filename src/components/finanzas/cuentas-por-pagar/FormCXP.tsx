...
import { Badge, Button, Checkbox, Col, Collapse, DatePicker, Descriptions, Divider, Drawer, Flex, Form, Grid, Input, Row, Select, Tooltip, Typography, Upload } from 'antd';
...
import { setDetalleDeCompra, setListaDeCompras, setLoading, setOpenDrawer, setRefresh, setTotalDeLaCompra, setIdNuevoProveedor } from "@/features/finanzasSlice";
...
import { setPerfilProveedores, setOpenDrawer as setOpenDrawerProveedor } from "@/features/ventasSlice";
          ...
          children: (
            ...
            <Upload
              ...
              onChange={({ fileList: newFileList }: any) => {
                settest((oldData: any) => ({ ...oldData, [key]: newFileList.slice(-2) }));
              }}
              beforeUpload={() => false}
            >
    ...
    [
      {
        ...
        children: <Typography.Text strong>
          {Number(totalDeTotales || detalleDeCompra?.totalDeLaCompra || 0).toLocaleString('es-MX', {
            style: 'currency',
            currency: 'MXN'
          })}
        </Typography.Text>// totalDeLaCompra,
      },
    ]
  ];
        ...
        onFinish={async (values) => {
          ...
          try {
            ...
            if (detalleDeCXP?.id) {// UPDATE
              ...
              await FireStoreFinanzas.registrarCXP(auth?.empresa?.id, {
                ...values,
                articulos: Boolean(values?.ordenDeCompra) ? detalleDeCompra?.articulos : articulos,
                gastosAdicionales: Boolean(values?.ordenDeCompra) ? detalleDeCompra?.gastosAdicionales : gastosAdicionales,
                totalDeLaCompra: Boolean(values?.ordenDeCompra) ? detalleDeCompra?.totalDeLaCompra : totalDeTotales,
              ...
              if (values?.solicitarFactura) {
                ...
                const findProveedor = listaDeProveedores.find(({ id: idProveedor }: any) => (idProveedor == values?.proveedor));
                await enviarEmail({
                  to: correo,
                  ...
                  montoFactura: Boolean(values?.ordenDeCompra) ? detalleDeCompra?.totalDeLaCompra : totalDeTotales,
                  conceptoCompra: values?.conceptoDeCompra || "",
                  listaArticulos: Boolean(values?.ordenDeCompra) ? detalleDeCompra?.articulos : articulos,
            ...
            Swal.fire({
              position: "top-end",
              icon: "success",
              title: `Compra por pagar ${detalleDeCompra?.id ? 'actualizada' : 'registrada'} con éxito!`,
      ...
      >
        ...
        <Row gutter={12}>
          ...
          <Col xs={24} sm={12} lg={8} xl={8}>
            <Form.Item name="codigo" label="Código" rules={[{ required: true, message: 'Código' }]}>
              <Input placeholder="Ingrese Código" maxLength={6} style={style} />
            </Form.Item>
          </Col>

          <Col xs={24} sm={12} lg={8} xl={8}>
            <Form.Item name="ordenDeCompra" label="Orden de compra" rules={[{ required: false, message: 'Seleccione Orden de compra' }]}>
              <Select onChange={(idCompra) => {
                const findOrdenDeCompra = listaDeCompras.find((compra: any) => {
                ...
                if (findOrdenDeCompra) {
                  ...
                  form.setFieldValue("fechaDeCompra", dayjs(findOrdenDeCompra?.fechaDeCompra, "YYYY-MM-DD"));

                  const proveedores = (findOrdenDeCompra?.articulos || []).map((articulo: any) => {
              ...
              ...COMPRAS]} placeholder="Seleccione Orden de compra" style={style} />
            </Form.Item>
          </Col>
          ...
          <Col xs={24} sm={24} lg={24} xl={24}>
            <Form.Item name="conceptoDeCompra" label="Concepto de compra" rules={[{ required: true, message: 'Concepto de compra' }]}>
              <Input.TextArea showCount maxLength={200} placeholder="Ingrese Concepto de compra" style={style} />
            </Form.Item>
          </Col>
        ...
        <Row gutter={12}>
          ...
          <Col xs={24} sm={12} lg={12} xl={12}>
            <Form.Item name="tipo" label="Tipo" rules={[{ required: true, message: 'Seleccione Tipo' }]}>
              ...
              }} options={TIPO} placeholder="Seleccione Tipo" style={style} />
            </Form.Item>
          </Col>
          <Col xs={24} sm={12} lg={12} xl={12}>
            ...
            <Form.Item valuePropName="checked" name="solicitarFactura" rules={[{ required: false, message: '' }]}>
              <Checkbox onChange={async (event: any) => {
                if (event.target.checked) {
                  const findProveedorSeleccionado = listaDeProveedores.find((proveedor: any) => {
                    return (proveedor?.id == form.getFieldValue("proveedor"));
                  });

                  const { value: email } = await Swal.fire({
                    inputValue: findProveedorSeleccionado?.correoContacto || findProveedorSeleccionado?.emailCompras || findProveedorSeleccionado?.emailFacturacion || "",
                    title: "Correo de facturación del cliente",
                    input: "email",
                    inputLabel: "Ingrese correo de facturación del cliente",
                    inputPlaceholder: "Ingrese correo de facturación del cliente",
