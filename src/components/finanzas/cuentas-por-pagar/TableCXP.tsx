...
import { Button, Dropdown, Modal, Table, TableProps, Tag, Tooltip, Typography } from 'antd';
import {
  EyeOutlined,
  DeleteOutlined,
  MoreOutlined,
...
import SkeletonCXP from './SkeletonCXP';
dayjs.extend(customParseFormat);



const TableCXP = ({ form }: any) => {
  ...
  const getMenuItems = (record: any): any[] => [
    ...
    },
    {
      ...
      label: (
        ...
        }}>{record?.tipo == "Factura" ? "Solicitar factura" : "Solicitar nota de crédito"}</a>
      ),
      icon: <CopyOutlined />,
  ...
  const columns: TableProps<any>['columns'] = [
    {
      title: <Typography.Title level={5}>{`Lista de cuentas por pagar, ${listaDeCXP.length} resultados`}</Typography.Title>,
      dataIndex: 'cxp',
      key: 'cxp',
      children: [
        {
        ...
        },
        {
          ...
          render: (text, record, index) => {
            const proveedor = listaDeProveedores.find((proveedor: any) => (proveedor?.id == text))?.nombreProveedor || "Desconocido";
            return (
              <div>{proveedor}</div>
            )
          },
        ...
        },
        {
          ...
          key: 'totalDeLaCompra',
          render: (text, record, index) => <div>{Number(text).toLocaleString('en-US')}</div>
        },
        {
          title: 'Fecha de pago',
        ...
        },
        {
          ...
          dataIndex: 'estatus',
          key: 'estatus',
          render: (text, record, index) => <Tag color={["Pagada"].includes(text) ? "success" : "processing"}>{text}</Tag>
        },
        {
          title: '',
          align: 'center',
        ...
        <div style={{ width: "100%" }}>
          <Button.Group>
            <Button
              ...
              block
              type="dashed"
              icon={<SendOutlined />}>Enviar</Button>
            <Button
              onClick={async () => {
                /* const a = document.createElement('a');
                a.href = doc?.url;
